...
}(function(ko, exports)
{
	...
	function validateSync(observable, rule, ctx)
	{
		...
		if (!rule.validator(observable(), ctx.params === undefined ? true : ctx.params))
	...
	ko.applyBindingsWithValidation = function(viewModel, rootNode, options)
	{
		...
		{ //have to figure out if they passed in a root node or options
...
/*
	Knockout Value Validation Extension Binding Handler 

	Description;
		This is a custom binding handler that extends the value binding handler of Knockout to utilise the Knockout validation
		extension library.
	...
	Usage;
		<input data-bind="value: Observable" data-val="true" data-val-length="The field must be a string with a maximum length of 10." data-val-length-max="10" data-val-regex="Not a valid value" data-val-regex-pattern="^[A-Z]{2}$" data-val-required="The MOA field is required." type="text" value="">
		<input data-bind="value: Observable, validate: OptionalBooleanOsbservable" data-val="true" data-val-length="The field must be a string with a maximum length of 10." data-val-length-max="10" data-val-regex="Not a valid value" data-val-regex-pattern="^[A-Z]{2}$" data-val-required="The MOA field is required." type="text" value="">
*/
$(function()
{
	// TODO: Remove "return false" and complete Knockout Validation implementation
	//return false;

	//ko.validation.makeBindingHandlerValidatable("bootstrap-typeahead"); 

	var validationAdapters = {
		required: function(object, attributes)
		{
		...
		number: function(object, attributes)
		{
			if (attributes["number"])
			{
				object.number = {
					params: true,
					message: attributes["number"]
				};
			}
		},
		range: function(object, attributes)
		{
			if (attributes["range"] && attributes["range-max"] && attributes["range-min"])
			{
				object.min = {
					params: parseInt(attributes["range-min"]),
					message: attributes["range"]
				};
				object.max = {
					params: parseInt(attributes["range-max"]),
					message: attributes["range"]
				};
			}
		},
		regex: function(object, attributes)
		{
			if (attributes["regex"] && attributes["regex-pattern"])
			{
				object.pattern = {
					params: attributes["regex-pattern"],
					message: attributes["regex"]
				};
			}
		},
		date: function(object, attributes)
		{
			if (attributes["date"])
			{
				object.date = {
		...
		//equalTo: function(object, attributes, viewModel)
		//{
		//	if (attributes["equalto"] && attributes["equalto-other"])
		//	{
		//		var other = attributes["equalto-other"].replace("*.", ""),
		//		    propCtx = utils.getPropertyContext(viewModel, other[0].toLowerCase() + other.substring(1));

		//		if (utils.isNotNullObject(propCtx))
		//		{
		//			object.equal = {
		//				params: propCtx.parentObj[propCtx.propertyName],
		//				message: attributes["equalto"]
		//			};
		//		}
		//	}
		//},
		length: function(object, attributes)
		{
			if (attributes["length"])
			{
				if (attributes["length-min"])
				{
					object.minLength = {
						params: parseInt(attributes["length-min"]),
				...
				{
					object.maxLength = {
						params: parseInt(attributes["length-max"]),
						message: attributes["length"]
					};
				}
			}
	...
	function AttachBindingHandlerValidation(bindingName)
	{
		var baseBinding = ko.bindingHandlers[bindingName];

		ko.bindingHandlers[bindingName] = {
			"init": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
			{
				var base = baseBinding.init(element, valueAccessor, bindingsAccessor, viewModel, bindingContext),
					koObservable = ko.isObservable(valueAccessor()) ? valueAccessor() : valueAccessor().data;
				...
				if ($(element).data("val"))
				{
					...
					$.each(element.attributes, function(index, item)
					{
						if (/^data-val/i.test(item.nodeName))
						{
							attributes[item.nodeName.replace(/^data-val-/i, "")] = $(element).attr(item.nodeName);
						}
					});

					$.each(validationAdapters, function(index, item)
					{
						item(validators, attributes);
					});

					koObservable.extend(validators);

					ko.utils.registerEventHandler(element, "blur", function()
					{
						ko.bindingHandlers[bindingName].validate(element, valueAccessor, bindingsAccessor, viewModel, bindingContext);
					});
					...
					$(element).tooltip(
					{
						title: function()
						{
							return $(element).data("originalTitle");
						}
					});
				}
			...
			//	if (baseBinding.update)
			//	{
			//		base = baseBinding.update(element, valueAccessor, bindingsAccessor, viewModel, bindingContext);
			...
			//	return base;
			//},
			"validate": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
			{
				if ($(element).data("val"))
				{
					var koObservable = ko.isObservable(valueAccessor()) ? valueAccessor() : valueAccessor().data,
						koError = ko.utils.peekObservable(koObservable.error),
					    koValid = ko.utils.peekObservable(koObservable.isValid),
					    koValidate = ko.utils.peekObservable(bindingsAccessor().validate);

					if ((koValidate || koValidate === undefined) && !koValid)
					{
						$(element).data("originalTitle", koError)
							.parent(".control-group")
							.addClass("error");
					}
					else
					{
						$(element).data("originalTitle", "")
							.parent(".control-group")
							.removeClass("error");
					}
				}
	...
	AttachBindingHandlerValidation("value");
	AttachBindingHandlerValidation("bootstrap-typeahead");
});