...
$(function()
{
	...
	var valueBinding = ko.bindingHandlers.value,
		validationAdapters = {
	...
	ko.bindingHandlers.value = {
		...
		{
			valueBinding.init(element, valueAccessor, bindingsAccessor, viewModel, bindingContext);
			...
			if ($(element).data("val"))
			{
				...
				valueAccessor().extend(validators);
				
				ko.utils.registerEventHandler(element, "blur", function()
				{
					ko.bindingHandlers.value.validate(element, valueAccessor, bindingsAccessor, viewModel, bindingContext);
				});
			}
		},
		"update": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
		{
			valueBinding.update(element, valueAccessor, bindingsAccessor, viewModel, bindingContext);
		...
		"validate": function(element, valueAccessor, bindingsAccessor, viewModel, bindingContext)
		{
			if ($(element).data("val"))
			{
				var koError = ko.utils.peekObservable(valueAccessor().error),
					koValid = ko.utils.peekObservable(valueAccessor().isValid),
				    koValidate = ko.utils.peekObservable(bindingsAccessor().validate);

				if (koValidate || koValidate === undefined)
				{
					if (koValid)
					{
						$(element).attr("title", "")
							.tooltip("destroy")
							.parent(".control-group").removeClass("error");
					}
					else
					{
						$(element).attr("title", koError)
