...
$.fn.dataTableExt.oApi.fnDisplayRow = function (oSettings, nRow) {
    ...
    for (var i = 0, iLen = oSettings.aiDisplay.length ; i < iLen ; i++) {
        if (oSettings.aoData[i].nTr._DT_RowIndex == nRow._DT_RowIndex) {
            iPos = i;
            break;
        }
    }
...
ko.bindingHandlers.dataTable3 = {
    'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        binding.itemClick = binding.itemClick || function () { };
        ...
        if (binding.columns && binding.columns.length) {
            ...
            ko.utils.arrayForEach(binding.columns, function (col) {
                ...
                //theIndex = binding.columns.indexOf(col); IE8 fix
                theIndex = $.inArray(col, binding.columns);
        ...
        if (binding.rowTemplate && binding.rowTemplate != '') {
            options.fnRowCallback = function(row, data, displayIndex, displayIndexFull) {
                ...
                data.ItemClick = function (e) { binding.itemClick(e); };

                ko.renderTemplate(binding.rowTemplate, data, null, row, "replaceChildren");

                if (row.innerText.replace(/[^a-zA-Z0-9]/g, '') == selectedRowString) {
        ...
        if (binding.captionTemplate) {
            binding.searchText = ko.observable("");
            binding.refreshTable = function () {
                local[binding.gridId].fnDraw(true);
            };
            
            ko.renderTemplate(binding.captionTemplate, binding, null, $(element).find("caption"), "replaceNode");
            options.fnServerParams = function (aoData) {
                var result = $.grep(aoData, function(e){ return e.name == "sSearch"; });
                if (result.length == 0) {
                    aoData.push({ "name": "sSearch", "value": binding.searchText() });
        ...
        if (options.bServerSide && options.bServerSide == true) {
                ...
                if (dataSource instanceof Array) {
                    ...
                    if (ko.isObservable(binding.dataSource)) {
                        ...
                        binding.dataSource.subscribe(function(newItems) {
                            ...
                            {
                                if ( $(tableNodes[i]).hasClass('row_selected') )
                                {
                                    aReturn.push( tableNodes[i] );
                                }
                            }
                            var currentTopRow = dataTable.fnSettings()._iDisplayStart;
                            if (aReturn.length == 1) {
                                selectedRowString = aReturn[0].innerText.replace(/[^a-zA-Z0-9]/g, '');
                            ...
                            //Code to set right page
                            dataTable.fnSettings()._iDisplayStart = currentTopRow;
                            dataTable.oApi._fnCalculateEnd(dataTable.fnSettings());
                            dataTable.fnDraw(false);
                            //Code to set right page
                            //if (local[binding.gridId]) {
                            //    aReturn = [];
                            //    for (var i = 0; i < local[binding.gridId].fnGetNodes().length; i++) {
                            //        if ($(local[binding.gridId].fnGetNodes()[i])[0].innerText.replace(/[^a-zA-Z0-9]/g, '') == selectedRowString) {
                            //            aReturn.push(local[binding.gridId].fnGetNodes()[i]);
                            //        }
                            //    }
                            //    if (aReturn.length == 1) {
                            //        local[binding.gridId].fnDisplayRow(aReturn[0]);
                            //    }
                            //}
                        });
                    }
        ...
        $(element).children().click(function (event) {
            ...
            var aTrs = local[binding.gridId].fnGetNodes();
            ...
            if (aReturn.length > 0) {
                if (options.bServerSide && options.bServerSide == true) {
                    binding.rowClick(local[binding.gridId].fnGetData()[[aReturn[0]._DT_RowIndex]]);
                } else {
                    binding.rowClick(ko.utils.unwrapObservable(binding.dataSource)[aReturn[0]._DT_RowIndex]);
                }
            };
