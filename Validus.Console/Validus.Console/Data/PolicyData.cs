...
namespace Validus.Console.Data
{
    public class PolicyData : IPolicyData
    {
        ...
        public COB GetCOB(string id)
        {
            return this._repository.Query<COB>().FirstOrDefault(c => c.Id == id);
        }

        public Quote GetQuote(string subscribeRef)
        {
            return this._repository.Query<Quote>().FirstOrDefault(c => c.SubscribeReference == subscribeRef);
        }
